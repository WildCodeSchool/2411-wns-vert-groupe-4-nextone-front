...
function ChooseService({ onBack, onNext, onCancel }: ChooseServiceProps) {
  ...
  >(
    ticket.serviceId
      ? { id: ticket.serviceId, name: ticket.serviceName || "" }
      : undefined
  );
  ...
  const { data: toggledData } = useSubscription(SERVICE_TOGGLED_SUBSCRIPTION);

  useEffect(() => {
    if (toggledData?.serviceToggled) {
      console.log("✅ Service toggled reçu:", toggledData.serviceToggled);

      if (
        selectedService?.id === toggledData.serviceToggled.id &&
        !toggledData.serviceToggled.isGloballyActive
      ) {
        console.log("⚠️ Le service sélectionné a été désactivé");
        setSelectedService(undefined);
        setTicket({ ...ticket, serviceId: "", serviceName: "" });
        setErrorMessage(
          "Le service que vous aviez sélectionné n'est plus disponible."
        );
      }
      refetch();
    }
  }, [toggledData, selectedService, refetch, ticket, setTicket]);
  ...
  function updateTicketService(
    ticket: TicketInfo,
    selectedService: { id: string; name: string } | undefined,
  ...
  const tabClass =
    "w-full text-[20px] text-primary py-6 rounded-md border border-primary text-center data-[state=active]:bg-primary data-[state=active]:text-white";
  ...
  if (activeServices.length === 0)
    return <Message text="Aucun service actif disponible." />;
